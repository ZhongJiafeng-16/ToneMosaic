...
class CommonClustering:
    ...
    def __call__(self, X, sample_nums):
        ...
        top_indeices_dict = self.center_sample_index(labels, X, sample_nums)
        return labels, top_indeices_dict
    ...
    def center_sample_index(self, labels, X, nums):
        # get the center sample index for each cluster
        cset = np.unique(labels)
        major_center = np.stack([X[labels == i].mean(0) \
            for i in cset])
        
        top_indeices_dict = {}
        for i in range(cset.shape[0]):
            lables_index = np.where(labels == cset[i])[0]
            cos_sim = cosine_similarity(X[lables_index], major_center[i][np.newaxis])
            sroted_indices = cos_sim.argsort(axis=0)
            top_n_indices = sroted_indices[-nums:]
            top_indices = lables_index[top_n_indices]
            top_indeices_dict[cset[i]] = top_indices

        return top_indeices_dict
