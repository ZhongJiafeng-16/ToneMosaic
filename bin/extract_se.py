...
from utils.spectrum import FBank
from utils.tools import dynamic_import
...
parser.add_argument('--wavs', default='/data/zhongjiafeng/Chinese/StarRail2.3/data', type=str, help='Wavs file folder')
parser.add_argument('--sample_nums', default=10000, type=int, help='sampling number of wavs, None is all')
...
CAMPPLUS_VOX = {
    'obj': 'modules.models.campplus.DTDNN.CAMPPlus',
    'args': {
        'feat_dim': 80,
        'embedding_size': 512,
    },
...
ERes2Net_VOX = {
    'obj': 'modules.models.eres2net.ERes2Net.ERes2Net',
    'args': {
        'feat_dim': 80,
        'embedding_size': 192,
    },
...
ERes2NetV2_COMMON = {
    'obj': 'modules.models.eres2net.ERes2NetV2.ERes2NetV2',
    'args': {
        'feat_dim': 80,
        'embedding_size': 192,
    },
...
ERes2Net_COMMON = {
    'obj': 'modules.models.eres2net.ERes2Net_huge.ERes2Net',
    'args': {
        'feat_dim': 80,
        'embedding_size': 192,
    },
...
ECAPA_CNCeleb = {
    'obj': 'modules.models.ecapa_tdnn.ECAPA_TDNN.ECAPA_TDNN',
    'args': {
        'input_size': 80,
        'lin_neurons': 192,
...
def main():
    ...
    try:
        wav_list = glob(f"{args.wavs}/**/*.wav", recursive=True)
        total_nums = len(wav_list)
        assert total_nums > 0
        if args.sample_nums is not None:
            wav_list = np.random.choice(wav_list, args.sample_nums, replace=False)
        print(f'[INFO]: Total {total_nums}, sampling {len(wav_list)} wav files.' + \
              f'Saved wavs list to {args.feat_out_dir.parent}')
        with open(f"{args.feat_out_dir.parent}/wav_list.txt", "w") as f:
            f.writelines([f"{wav}\n" for wav in wav_list])
    ...
    pretrained_state = torch.load(pretrained_model, map_location='cpu', weights_only=True)
    model = conf['model']
